...
def create_widgets_in_third_frame():
        ...
        def week():                          # Analyse your tweets and classifies them into class which the tweets falls
         ...
         for i in range(len(files.Max_tweet)):
          if(len(files.Max_tweet[i])>1):
            if(files.Max_tweet[i]==a[0] or files.Max_tweet[i]==a[1]):
             c+=1
         #if(c>2):
         call_fourth_frame_on_top()      
         '''
        ...
        #Past_Analysis_button = Button(third_frame, text="Past Analysis",bg='#D1DDDB',fg='black', command=past)
        #Past_Analysis_button.config(font=('Ariel',12))
        #Past_Analysis_button.pack(pady=10)
